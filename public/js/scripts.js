...
$('#sort').on('change',function(e){
  ...
  $.get('/ajax-sortproducts?sort=' +  sort, function(data){
      ...
      $('.productsAjax').append('<div class="col-md-3"><div class="thumbnail"> <a href="store/viewproduct/'+productsObj.id +'"> <div class="caption clearfix"> <h4>' + productsObj.title +
        '</h4><p>'+productsObj.description+'</p><div id="viewproductdetailed"> <button class="btn btn-info btn-xs modalproduct" >View Product</button></div></div></a><img src="img/products/' + productsObj.image+
   ...
   sliderDiv.slider({ range: true, 
    ...
    stop: function (event, ui) {
       ...
       $.ajax({
            type: "POST",
            url:urlPrice,
            data: {'min_price':min ,'max_price':max , _token:token},
            success: function(data){
                   $('.productsAjax').html(data);

    ...
    public function Ajaxprice(Request $request){
      ...
      if($request->ajax()){
        ...
        $brands = Input::has('brands') ? Input::get('brands'):[];
           ...
           if($brands){
                        foreach ($brands as  $brand) {
                        $products=  Product::orWhere('price', '>=', $request->min_price)
                        ->where('price', '<=' , $request->max_price)
                        ->where('brand_id', '=' , $brand)->take(8)->get();                      
                            
                }
          }
         $products = Product::where('price', '>=' , $request->min_price)
            ->where('price', '<=' , $request->max_price)->take(8)->get();
